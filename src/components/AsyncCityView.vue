    ...
    <div class="flex flex-col flex-1 items-center">
        ...
        <div class="max-w-screen-md w-full py-12">
            <div class="mx-8 text-white">
                ...
                <div class="flex gap-10 overflow-x-auto scrollbar-hide">
                    <div v-for="hourData in weatherData.hourly" :key="hourData.dt" class="flex flex-col gap-4 items-center">
                        ...
                        <img class="w-auto h-[50px] object-cover" :src="`http://openweathermap.org/img/wn/${hourData.weather[0].icon}@2x.png`
                                " alt="" />
                        <p class="text-xl">
                            {{ Math.round(hourData.temp - 273.15) }}&deg;C
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="mx-8 text-white">
                ...
                <div v-for="day in weatherData.daily" :key="day.dt" class="flex items-center">
                    ...
                    <div class="flex gap-2 flex-1 justify-end">
                        <p>H: {{ Math.round(day.temp.max - 273.15) }}&deg;C</p>
                        <p>L: {{ Math.round(day.temp.min - 273.15) }}&deg;C</p>
                    </div>
                </div>
            </div>
        </div>
...
const getWeatherData = async () => {
    try {
        const weatherData = await axios.get(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=a23f253ca79298a623ef22cce1483768`
        );
